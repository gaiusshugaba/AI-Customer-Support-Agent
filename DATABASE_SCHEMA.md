...
**Note:** Every `request_errors` row should have a non-null `tenant_id`. Rows with `null` or `'unknown'` tenant IDs are invisible to the tenant-scoped admin console and should be considered a bug in the writer.
