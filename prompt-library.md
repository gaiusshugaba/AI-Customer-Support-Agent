...
Six LLM prompts total: two in the `[RAG]` path (every conversation turn), four in the `[ESCALATION]` path (only when a case is active). A seventh path — the **fast path** — deliberately skips the LLM entirely for trivially-short messages and returns a canned response, so it has no prompt.
...
**Runs:** every conversation turn, **except** those intercepted by `[RAG] - Is Trivial Message?` (short messages with no intent/risk keywords skip the classifier entirely).
