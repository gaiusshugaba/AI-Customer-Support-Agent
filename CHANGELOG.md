...
- **Minimum-chunk threshold (`MIN_CHARS = 100`)** so preamble/metadata sections that are too short to embed do not become orphaned vectors.

- **Explicit `SKIP_HEADINGS` filter** for the `## Document metadata` section, which contains useful metadata but is too small to embed reliably on its own.

- **Chunk context enrichment:** every chunk now carries a `text` field containing `title — section_heading\n\nbody`, which is the field actually embedded. Previously the loader was reading the wrong field and embedding only the heading.
...
- **Public RLS SELECT policies** on `ingestion_log` and `request_errors` so the admin console can read them via the anon key without disabling RLS entirely.
...
- **`doc_id` resolution now uses the full filename** (minus extension) rather than a positional split, preventing collisions when two files share a word (e.g., `01-product-overview` vs `10-product-status`).

- **`tenant_id` now mandatory on every error record.** Previously `request_errors` rows were written with `tenant_id = null` or `'unknown'`, which made them invisible to the frontend's tenant-scoped query. Both the workflow-level error handler and the inline error loggers were patched to fall back to `'b2b-saas-demo'` and to extract the real tenant from the failed execution data when available.
...
- **`request_errors` table invisible in the admin console.** Rows were being written but filtered out by `eq("tenant_id", 'b2b-saas-demo')` because the tenant value was null. Fixed both the historical rows (SQL UPDATE) and the workflow paths that produce them.
