...
## \## Phase 4 — Ingestion Robustness, Multi-Format Support \& Latency Optimization

## 

## \### Added

## \- \*\*Multi-format chunker with dual-strategy heading detection.\*\* Markdown files use literal `#`/`##`/`###` splits; PDF/DOCX/plain-text files fall back to a heuristic that detects headings via short line length, no terminal punctuation, and question-word patterns. Both strategies are implemented in a single `\[INGEST] - Chunk Document` node.

## \- \*\*Minimum-chunk threshold (`MIN\_CHARS = 100`)\*\* so preamble/metadata sections that are too short to embed do not become orphaned vectors.

## \- \*\*Explicit `SKIP\_HEADINGS` filter\*\* for the `## Document metadata` section, which contains useful metadata but is too small to embed reliably on its own.

## \- \*\*Chunk context enrichment:\*\* every chunk now carries a `text` field containing `title — section\_heading\\n\\nbody`, which is the field actually embedded. Previously the loader was reading the wrong field and embedding only the heading.

## \- \*\*Fast path for trivial messages.\*\* A deterministic regex classifier (`\[RAG] - Is Trivial Message?`) intercepts greetings, thanks, and acknowledgements before any LLM call, saving \~2 seconds on chat turns like "hi", "thanks", "ok".

## \- \*\*Merge node (`\[CHAT] - Merge Context Loads`)\*\* to parallelize the three Supabase context lookups (tenant config, customer context, conversation history) while ensuring `Build Conversation Context` only fires after all three complete.

## \- \*\*Public RLS SELECT policies\*\* on `ingestion\_log` and `request\_errors` so the admin console can read them via the anon key without disabling RLS entirely.

## \- \*\*Error Workflow isolation.\*\* The `\[ERROR] - \*` nodes now live in a separate workflow that only accepts errors from the main platform workflow, verified by workflow ID.

## 

## \### Changed

## \- \*\*Embedding provider: Gemini → OpenAI.\*\* Ingestion and query embeddings now use `openai/text-embedding-3-large` (3072 dimensions) routed through OpenRouter. The previous Gemini embeddings node was producing inconsistent results and rate-limiting under free-tier quota during bulk ingestion. Pinecone index was retained at 3072 dimensions, which matches the OpenAI model.

## \- \*\*`\[INGEST] - Build Pinecone Documents` switched to "Load Specific Data" mode\*\*, explicitly reading the `text` field. Previously fell back to a metadata field (usually `version`) as the embedded content, which is why early retrieval returned perfect match scores on empty headings.

## \- \*\*`\[INGEST] - Normalize Document` regex expanded\*\* from `\\|?` to `\[-•|]?` so it correctly parses bullet-prefixed metadata lines (`- Title: ...`) alongside pipe-prefixed ones.

## \- \*\*`doc\_id` resolution now uses the full filename\*\* (minus extension) rather than a positional split, preventing collisions when two files share a word (e.g., `01-product-overview` vs `10-product-status`).

## \- \*\*`tenant\_id` now mandatory on every error record.\*\* Previously `request\_errors` rows were written with `tenant\_id = null` or `'unknown'`, which made them invisible to the frontend's tenant-scoped query. Both the workflow-level error handler and the inline error loggers were patched to fall back to `'b2b-saas-demo'` and to extract the real tenant from the failed execution data when available.

## 

## \### Fixed

## \- \*\*Chunk count discrepancy on PDFs.\*\* The old chunker only split on `##`/`###`, so PDFs (which have no markdown markers) collapsed to a single chunk per heading-less body. This caused retrieval to fail on documents whose answers were spread across many short sections. The heuristic fallback solved this — a 19-section PDF now produces 17+ chunks instead of 4.

## \- \*\*Pinecone "Vector dimension 0" error.\*\* Was caused by the loader embedding a metadata field that contained only `"2.0"` — the OpenAI model returned an empty vector for that input, which Pinecone rejected. Root cause was the Document Loader reading the wrong field; the fix (setting it explicitly to `text`) resolved the issue without changing the model.

## \- \*\*Frontend "no documentation available" responses despite successful ingestion.\*\* Retrieval was returning correct scores (0.92+) but with empty `pageContent`, so the LLM had nothing to ground its answer in. Same root cause as above; fixed by ensuring the `text` field contains the full paragraph, not just a heading or metadata value.

## \- \*\*`request\_errors` table invisible in the admin console.\*\* Rows were being written but filtered out by `eq("tenant\_id", 'b2b-saas-demo')` because the tenant value was null. Fixed both the historical rows (SQL UPDATE) and the workflow paths that produce them.
...
* **Case Relevance Classifier (`\[ESCALATION] - Evaluate Message Against Active Case`):** Determines whether a customer's message continues an existing `awaiting\_customer\_info` case or represents a new support inquiry. Prevents active cases from trapping unrelated customer questions.
* **Deterministic Multi-Stage Guardrail Router:**

  1. Risk flag evaluation (`security`, `billing\_dispute`, `legal`, `churn\_threat`)
...
* **Case state machine:** `AWAITING\_CUSTOMER\_INFO` → `READY\_FOR\_AGENT`, persisted in Supabase and tracked across multiple messages in the same conversation.
...
* **Deterministic Ready Response Node:** generates consistent customer-facing confirmations when tickets reach `READY\_FOR\_AGENT`.
...
* **Case ID Loss on Follow-Ups:** Replaced loose variable lookups with strict fallbacks across `intake\_context`, `active\_escalation\_case`, and parent JSON scopes.
...
* Request validation (`tenant\_id`, `conversation\_id`, message text) before any processing begins; invalid requests are rejected and logged.
...
* Duplicate-ingestion handling: before re-embedding a document, the pipeline checks Supabase for a prior successful ingestion of that `doc\_id` and deletes the corresponding vectors from Pinecone (filtered by `doc\_id` within the tenant's namespace) before re-ingesting.
...
* An aggregation node was losing `doc\_id`/`tenant\_id`/`title` because a Vector Store node's output doesn't reliably pass through custom fields from its input — fixed by having the aggregation step reach back to the chunking node by name.
...
* `\[INGEST] - Unwrap Request` was discarding the uploaded file binary, assuming a JSON payload with `file\_content`. Fixed to preserve `$binary` so multipart/form-data uploads flow through correctly.
...
* `\[INGEST] - Extract PDF Text` (n8n's built-in Extract From File node) outputs extracted text to `$json.text`, but `\[INGEST] - Validate Extracted Text` checks `$json.extracted\_text`. Fixed by having `\[INGEST] - Normalize Document` normalize both fields into `extracted\_text`.
* `\[INGEST] - Normalize Document` was not parsing document metadata embedded in the text content (e.g., `Document ID`, `Tenant`, `Category`, `Version`, `Last Updated` headers inside PDFs/Markdown). It also failed to derive `file\_type` when `$binary.file.fileExtension` was empty after extraction. Fixed with a hybrid resolver: text metadata > request body > filename parsing > MIME type inference.
